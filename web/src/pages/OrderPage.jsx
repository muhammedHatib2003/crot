    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <header className="mb-6 rounded-3xl bg-white p-6 shadow-md">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">QR Table Ordering</p>
            <h1 className="mt-2 text-3xl font-bold text-brand-900">{payload?.restaurant?.name}</h1>
            <p className="mt-1 text-sm text-slate-600">
              Ordering for table {payload?.table?.name} with {payload?.table?.seats} seats
            </p>
          </div>
          <div className="rounded-2xl bg-brand-50 px-4 py-3 text-sm text-brand-900">
            <p className="font-semibold">Table status</p>
            <p>{payload?.table?.status}</p>
          </div>
        </div>
      </header>
          ...
          {activeOrder ? (
            <section className="rounded-3xl bg-white p-5 shadow-md">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-slate-500">Latest order</p>
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="rounded-2xl bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-900">{activeOrder.customerName || "Guest"}</p>
                </div>
                <div className="rounded-2xl bg-slate-50 p-3">
              ...
              {activeOrder.payment ? (
                <div className="mt-4 rounded-2xl border border-green-200 bg-green-50 p-3 text-sm text-green-900">
                  Payment received via {activeOrder.payment.paymentMethod}. Receipt {activeOrder.payment.receiptCode}.
                </div>
              ) : null}
            ...
            <section className="rounded-3xl bg-white p-5 shadow-md">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold text-slate-900">Choose a category</h2>
                  <p className="mt-1 text-sm text-slate-500">Tap drinks, food, desserts, or any other section to see only those items.</p>
                </div>
              ...
              <div className="mt-6">
                <div className="mb-4 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-slate-900">{activeCategory}</h3>
                  </div>
                  ...
                  {visibleItems.map((item) => {
                    ...
                    return (
                      <article key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                        {item.photoUrl ? (
                          <img alt={item.name} className="h-44 w-full object-cover" src={item.photoUrl} />
                        ) : (
                          <div className="flex h-44 w-full items-center justify-center bg-slate-200 text-xs text-slate-500">
                            No Photo
                          </div>
                        )}
                        <div className="p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-2 text-sm text-slate-600">{item.description || "Chef special selection."}</p>
                            </div>
                          ...
                          <div className="mt-4 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2">
                              <button
                                type="button"
                                className="h-9 w-9 rounded-full border border-slate-300 text-lg text-slate-700 hover:bg-slate-100"
                            ...
                            <button
                              type="button"
                              className="rounded-full bg-brand-700 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-900"
                              onClick={() => increaseQuantity(item.id)}
                            >
        ...
        <aside>
          <div className="rounded-3xl bg-white p-5 shadow-md xl:sticky xl:top-6">
            ...
            <div className="mt-4 space-y-3">
              {cartItems.map((item) => (
                <div key={item.id} className="rounded-2xl border border-slate-200 p-3">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      type="button"
                      className="h-8 w-8 rounded-full border border-slate-300 text-slate-700 hover:bg-slate-100"
                      onClick={() => decreaseQuantity(item.id)}
                    >
                      -
                    </button>
              ...
              {cartItems.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-300 p-4 text-sm text-slate-500">
                  Your cart is empty. Add products from the menu to place an order.
                </div>
              ) : null}
            </div>
            ...
            <form className="mt-5 space-y-3" onSubmit={handleSubmit}>
              ...
              >
                {submitting ? "Sending order..." : `Place order for ${formatPrice(cartTotal)}`}
              </button>
            </form>
          </div>
        </aside>
