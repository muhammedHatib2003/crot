    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      <header className="mb-6 rounded-2xl bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">{t("myOrders.title")}</h1>
            <p className="mt-1 text-sm text-slate-600">{t("myOrders.description")}</p>
          </div>
          {onLogout ? (
            <button
      ...
      {loading ? <div className="rounded-2xl bg-white p-6 text-slate-600 shadow-sm">{t("myOrders.loading")}</div> : null}
      ...
      {!loading && orders.length === 0 ? (
        <div className="rounded-2xl bg-white p-6 text-slate-600 shadow-sm">
          {t("myOrders.empty")}{" "}
          <Link className="text-brand-700 hover:text-brand-900" to="/online-order">
            {t("myOrders.viewRestaurants")}
          </Link>
        </div>
      ) : null}
          ...
          <article key={order.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h2 className="text-lg font-semibold text-slate-900">{order.restaurant?.name || t("myOrders.restaurantFallback")}</h2>
